...
function editProfile(event) {
  ...
  const direccion = document.getElementById('edit-address').value;
  const telefono = document.getElementById('edit-phone').value;
  const ciudad = document.getElementById('edit-city').value;
  const zip = document.getElementById('edit-zip').value;
  const genero = document.querySelector('input[name="genero"]:checked')?.value || 'O';
  ...
  document.getElementById('resumen-zip').textContent = zip;

  document.getElementById('resumen-genero').textContent =
    genero === 'M' ? 'Masculino' : genero === 'F' ? 'Femenino' : 'Otro';


  // Aquí podrías enviar los datos a un backend con fetch(), por ahora solo simulamos
  console.log("Guardando perfil...");
  console.log("Nombre:", name);
  console.log("Email:", email);
  console.log("Contraseña:", password);

  // Volver a vista resumen
